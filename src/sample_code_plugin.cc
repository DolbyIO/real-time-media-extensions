...
using namespace dolbyio::comms::transcription;
using namespace dolbyio::comms::audio_utils;

class speech_to_text_writer
    : public dolbyio::comms::transcription::transcription_listener {
 public:
...
class my_interactor : public dolbyio::comms::sample::interactor {
 public:
  my_interactor(commands_handler_interface& hdl) : hdl_(hdl) {
    std::vector<std::shared_ptr<transcription_service::factory>> factories;
    factories.push_back(gladia::factory());
    factories.push_back(aws_wrapper::factory());
 ...
 private:
  ...
  void do_start() {
    ...
    std::shared_ptr<file_writer> outfile{};
    if (!transcript_fifo.empty()) {
      std::ofstream f;
      f.open(transcript_fifo, std::ios::out | std::ios::binary);
      outfile = file_writer::create(
          std::move(f), std::make_unique<file_writer::binary_formatter>());
    } else if (!transcript_file.empty()) {
      std::ofstream f;
      f.open(transcript_file, std::ios::out | std::ios::binary);
      outfile = file_writer::create(
          std::move(f), std::make_unique<file_writer::json_formatter>());
    }

    signal_handler_ = std::make_unique<signal_handling_helper>(&hdl_);
    speech_ = live_transcription::create(
        *sdk_, live_transcription::config{engine_config_},
        transcription_service_factories_[transcript_service],
        std::make_shared<speech_to_text_writer>(*sdk_, signal_handler_.get(),
                                                outfile));
  }
...
class my_plugin_structure : public dolbyio::comms::sample::plugin_structure {
 public:
  void start_plugin(commands_handler_interface& handler) override {
    if (interactor_)
      throw std::runtime_error("Already started");

    interactor_ = std::make_shared<my_interactor>(handler);
 ...
 private:
  std::shared_ptr<my_interactor> interactor_{};
};

my_plugin_structure g_plugin;

extern "C" {
TRANSCRIPTION_EXPORT dolbyio::comms::sample::plugin_structure*
